    ...
    for (const modelName of modelsToTry) {
      ...
      for (let attempt = 1; attempt <= attempts; attempt++) {
        try {
          ...
          if (isQuotaExceeded) {
            console.log(`[Gemini] Cota excedida no modelo ${modelName}. Pulando imediatamente para o próximo modelo para evitar esperas.`);
            break; // Break the attempt loop to move on to the next model instantly
          } else if (isHighDemand) {
            console.log(`[Gemini] Modelo ${modelName} indisponível ou sob alta demanda. Pulando para o próximo modelo.`);
            break; // Break the attempt loop to move on to the next model instantly
          } else {
            if (attempt < attempts) {
